...
def category():
    ...
    books = conn.execute("""
        SELECT * FROM book WHERE categoryId = ? ORDER BY title
...
def search():
    ...
    """).fetchall()
    books = conn.execute("""
        SELECT * FROM book WHERE lower(title) LIKE lower(?) ORDER BY title
    """, (f"%{term}%",)).fetchall()
    conn.close()
    return render_template(
        "category.html",
...
def featured():
    ...
    """).fetchall()
    books = conn.execute("""
        SELECT * FROM book WHERE readNow = 1 ORDER BY title
    """).fetchall()
    conn.close()
    return render_template(
        "category.html",
        categories=categories,
        selectedCategory={"categoryName": "Featured Books (Read Now)"},
        books=books,
        searchTerm=None,
        nothingFound=(len(books) == 0)
    )
